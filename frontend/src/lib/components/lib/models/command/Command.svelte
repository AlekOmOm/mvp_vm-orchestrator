...
import ExecuteButton from './buttons/ExecuteButton.svelte';
...
const currentExecutionStore = commandExecutor.getCurrentExecution();
const currentExecution = $derived($currentExecutionStore);
const isCurrentCommand = $derived(currentExecution?.command?.cmd === command.cmd);
</script>
