...
export class VMService {
  ...
  async ensureRegistered(alias) {
    ...
    try {
      ...
      console.log('📋 Backend VMs:', backendVMs);
      const existing = backendVMs.find((vm) => vm.alias === alias || vm.name === alias);
      if (existing) {
    ...
    if (!sshHost) {
      throw new VMServiceError(`SSH host '${alias}' not found – cannot register VM`);
    }
  ...
  async validateSSHHost(vmId) {
    try {
      ...
      throw new VMServiceError(`SSH host validation failed for ${vmId}`, error);
    }
  }
  ...
  async executeCommand(vmId, command, options = {}) {
    ...
    try {
      ...
      throw new VMServiceError(`Failed to execute command on ${vmId}`, error);
    }
  }
